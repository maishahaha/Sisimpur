        ...
        .then((data) => {
          if (data.success) {
            ...
            if (data.error && data.error.includes("already subscribed")) {
              responseDiv.innerHTML =
                '<div class="alert alert-info">This email is already in our list. Thank you!</div>';
            } else {
              responseDiv.innerHTML =
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (submitBeta && betaForm) {
        submitBeta.addEventListener('click', function() {
            if (!betaForm.checkValidity()) {
                event.preventDefault();
                event.stopPropagation();
                betaForm.classList.add('was-validated');
                return;
            }
            ...
            const modalBody = betaModal.querySelector('.modal-body');
            modalBody.innerHTML = `
                <div class="alert alert-success">
                    Thank you for applying to be a beta tester! We'll be in touch soon.
                </div>
            `;
            
            const modalFooter = betaModal.querySelector('.modal-footer');
    ...
    if (betaModal) {
        betaModal.addEventListener('hidden.bs.modal', function() {
            if (betaForm) {
                betaForm.reset();
                betaForm.classList.remove('was-validated');
            }
        });
    }
});
...
document
  .querySelectorAll("#betaForm input, #betaForm select")
  .forEach((input) => {
