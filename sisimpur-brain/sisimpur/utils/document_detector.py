...
def detect_language(text: str) -> str:
    ...
    if (bengali_chars >= 5 and 
        (ratio > 0.07 or 
         any(keyword in text for keyword in bengali_keywords))):
        return "bengali"
...
def detect_question_paper(text: str, language: str) -> bool:
    ...
    bengali_terms = [
        "প্রশ্ন", "উত্তর", "পরীক্ষা", "নম্বর", "মোট নম্বর",
        "সময়", "ঘন্টা", "মিনিট", "পাঠ্যক্রম", "বিষয়", "পর্ব"
    ]
    ...
    if language != "bengali":
        ...
        english_terms = [
            "question", "answer", "exam", "test", "marks",
            "points", "score", "time", "minutes", "hours",
            "total marks", "section"
        ]
        
        term_matches = sum(1 for term in english_terms if term in text_lower)
        
        if (len(question_numbers) >= 3 or 
            len(mcq_options) >= 5 or 
            term_matches >= 2):
...
def detect_document_type(file_path: str) -> Dict[str, Any]:
    ...
    try:
        if file_ext == '.pdf':
            ...
            if (is_scanned_pdf or 
                total_images >= len(doc) or 
