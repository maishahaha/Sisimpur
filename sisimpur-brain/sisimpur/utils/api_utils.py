...
class RateLimitedAPI:
    ...
    def get_model(self, model_name: str = DEFAULT_GEMINI_MODEL) -> genai.GenerativeModel:
    ...
    def with_rate_limit(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if self.request_count >= RATE_LIMIT_BATCH_SIZE:
            ...
            if time_since_cooldown < RATE_LIMIT_COOLDOWN:
                ...
                logger.info(f"Rate limit cooldown: sleeping for {cooldown_time:.2f} seconds")
    ...
    def generate_content(
        ...
        try:
            ...
            if fallback and model_name != FALLBACK_GEMINI_MODEL:
                logger.warning(f"Falling back to {FALLBACK_GEMINI_MODEL} due to rate limits")
