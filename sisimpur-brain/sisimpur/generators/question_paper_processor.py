...
def process_llm_response(json_str: str, max_questions: Optional[int] = None) -> List[Dict[str, Any]]:
    """
        ...
        if "questions" in qa_data and isinstance(qa_data["questions"], list):
            ...
            for q in questions:
                if "question" in q:
                    ...
                    if QUESTION_TYPE == "MULTIPLECHOICE" and ("options" not in q or not isinstance(q["options"], list)):
    ...
    def process(self, text: str, max_questions: Optional[int] = None) -> List[Dict[str, str]]:
        ...
        try:
            ...
            try:
                ...
                response = api.generate_content(prompt_template, model_name=QA_GEMINI_MODEL)
            except Exception as e:
                logger.warning(f"Error with primary model, trying fallback: {e}")
                logger.info("Processing question paper with fallback model")
                response = api.generate_content(prompt_template, model_name=FALLBACK_GEMINI_MODEL)
